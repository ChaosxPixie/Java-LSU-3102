...
public class Graph<E extends Comparable<E>> implements GraphAPI<E>
{
    ...
    {
        /* implement this method
        check to see if traversal method (bfs or dfs) reaches a dest
        if yes = true no = false */
        //check for existance of source vertex
        Vertex tmpFrom = first;
        while (tmpFrom != null && cmp.compare(fromKey,tmpFrom.data)>0)
            tmpFrom = tmpFrom.pNextVertex;
        if (tmpFrom == null || cmp.compare(fromKey,tmpFrom.data) != 0)
        ...
        //set all vertices starting with the from vertex to 0
        Vertex walkPtr = first;
        while (walkPtr != null)
        {
            walkPtr.processed = 0;
        ...
        //traverse vertex and return true if meet a vertex with data that matches
        //the toKey
        ...
        while (walkPtr != null)
        {
            ...
            {
                tmp = queue.remove(0);
                if(cmp.compare(toKey, tmp.data) == 0)
                {
                    return true;
                }
                tmp.processed = 2;
    ...
    public long countEdges()
    {
        ...
        while (tmpV.pNextVertex != null){
            out += tmpV.pNextVertex.outDeg;
            tmpV = tmpV.pNextVertex;
        }
        return out;
    }
