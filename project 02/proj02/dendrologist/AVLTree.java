...
public class AVLTree<E extends Comparable<E>> implements AVLTreeAPI<E>
{
    ...
    public boolean isFibonacci()
    {
        ...
        if ((fibonacci((height()+2)-1) == count || count < 3) && (height() != BalancedFactor.RH.value))
            return true;
        else
            return false;
    }
    ...
    private int fullCount(Node node)
    {
        ...
        if ((node.left != null && node.right != null))
            numFull++;
            numFull += (fullCount(node.right) + fullCount(node.left));
        return numFull;
    }
    /**
